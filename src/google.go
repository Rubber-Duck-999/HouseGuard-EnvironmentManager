...
func google_drive() {
        ...
        config, err := google.ConfigFromJSON(b, appsactivity.ActivityScope)
        if err != nil {
        ...
        if len(r.Activities) > 0 {
                for _, a := range r.Activities {
                        event := a.CombinedEvent
                        when := time.Unix(0, int64(event.EventTimeMillis))
                        user := event.User
                        target := event.Target
                        if user == nil || target == nil {
                                continue
                        }
                        fmt.Printf("%s: %s %s %s %s\n", when.Format(time.RFC822), user.Name,
                                event.PrimaryEventType, target.Name, target.MimeType)
                }
        } else {
                fmt.Print("No recent activity.")
        }
